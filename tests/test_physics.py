...
class TestCallOptionPhysics:
    ...
    def test_boundary_lower(self, call_engine: CallOption) -> None:
        ...
        assert torch.all(val == 0), "Lower boundary for Call Option must be exactly 0" # Validates that the condition is True; raises an AssertionError if False.
    ...
    def test_analytical_accuracy(self, call_engine: CallOption) -> None:
        ...
        np.testing.assert_allclose(
            project_prices, ref_prices, 
            rtol=1e-5, atol=1e-5, # rtol handles large values (e.g., high asset price), atol handles values near zero (e.g., OTM price)
...
class TestPutOptionPhysics:
    ...
    def test_boundary_lower(self, put_engine: PutOption) -> None:
        ...
        assert torch.isclose(
            val_project, 
            torch.tensor(val_expected, dtype=torch.float32), 
            atol=1e-5
...
def test_tensor_numpy_compatibility(call_engine: CallOption) -> None:
    ...
    assert isinstance(res_np, np.ndarray), "Failed to return numpy array for numpy input"
    ...
    # Value Check
    assert np.isclose(res_np.item(), res_torch.item()), "Inconsistent results between Numpy and Tensor"