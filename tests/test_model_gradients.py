    ...
    def test_weight_initialization(self, model: UniversalPINN) -> None:
        ...
        for name, param in model.named_parameters():
            if "weight" in name:
                assert (
                    torch.std(param) > 0
                ), f"Layer {name} has zero variance (not initialized?)"
...
class TestGradientFlow:
    ...
    def test_autograd_enablement(self, model: UniversalPINN) -> None:
        ...
        for param in model.parameters():
            assert (
                param.requires_grad
            ), "Model parameter is frozen (requires_grad=False)"
    ...
    def test_first_derivative_computation(self, model: UniversalPINN) -> None:
        ...
        assert (
            grads.grad_fn is not None
        ), "Gradient computation graph is broken (detached)"
        assert not torch.isnan(grads).any(), "First derivative contains NaNs"

    def test_pde_residual_execution(
        self, model: UniversalPINN, physics_engine: CallOption
    ) -> None:
        """
        ...
        assert (
            pde_residual.grad_fn is not None
        ), "PDE residual lost connection to computational graph!"
