...
from src.physics.call_option import CallOption  # Used as a concrete implementation for testing
...
class TestMarketNormalizer:
    ...
    def test_normalization_range_integrity(self, normalizer: MarketNormalizer) -> None:
        ...
        norm_min = normalizer.normalize_batch(
            min_vals[:, 0:1], min_vals[:, 1:2], min_vals[:, 2:3], min_vals[:, 3:4], min_vals[:, 4:5]
        )
        np.testing.assert_allclose(norm_min, 0.0, atol=1e-7, err_msg="Min values must normalize to 0.0")
        ...
        norm_max = normalizer.normalize_batch(
            max_vals[:, 0:1], max_vals[:, 1:2], max_vals[:, 2:3], max_vals[:, 3:4], max_vals[:, 4:5]
        )
        np.testing.assert_allclose(norm_max, 1.0, atol=1e-7, err_msg="Max values must normalize to 1.0")
    ...
    def test_reversibility_round_trip(self, normalizer: MarketNormalizer) -> None:
        ...
        np.testing.assert_allclose(t, t_rec, atol=1e-7, err_msg="Time (t) failed round-trip")
        np.testing.assert_allclose(S, S_rec, atol=1e-7, err_msg="Spot (S) failed round-trip")
        np.testing.assert_allclose(sigma, sig_rec, atol=1e-7, err_msg="Sigma failed round-trip")
        np.testing.assert_allclose(r, r_rec, atol=1e-7, err_msg="Risk-free rate (r) failed round-trip")
        np.testing.assert_allclose(K, K_rec, atol=1e-7, err_msg="Strike (K) failed round-trip")
    ...
    def test_price_normalization_logic(self, normalizer: MarketNormalizer) -> None:
        ...
        np.testing.assert_allclose(norm_res, expected_norm, err_msg="Price normalization calculation wrong")
        
        # Test Denormalize (Reverse)
        rec_price = normalizer.denormalize_price(norm_res, K)
        np.testing.assert_allclose(rec_price, price, err_msg="Price denormalization failed round-trip")
...
class TestDataGenerator:
    ...
    def test_pde_batch_shape(self, data_generator: DataGenerator) -> None:
        ...
        assert batch.shape == (n_samples, 5), f"Expected shape ({n_samples}, 5), got {batch.shape}"
        
        # Check range roughly [0, 1]
        assert batch.min() >= -0.1 and batch.max() <= 1.1, "Normalized data drifted too far from [0,1]"
    ...
    def test_ivp_batch_logic(self, data_generator: DataGenerator) -> None:
        ...
        t_col = X_ivp[:, 0]
        np.testing.assert_allclose(t_col, 0.0, atol=1e-9, err_msg="IVP batch must have t=0")
        
        # Check y shape (Option Price)
        assert y_ivp.shape == (n_samples, 1)

    def test_kink_batch_hard_attention(self, data_generator: DataGenerator, normalizer: MarketNormalizer) -> None:
        """
        [Research-Grade Test]
        ...
        np.testing.assert_allclose(t_col, 0.0, atol=1e-9, err_msg="Kink batch must be at maturity (t=0)")
        ...
        np.testing.assert_allclose(S_real, K_real, atol=1e-5, err_msg="Kink batch must enforce S = K")
    ...
    def test_discrete_k_sampling(self, data_generator: DataGenerator, normalizer: MarketNormalizer) -> None:
        """
        ...
        np.testing.assert_allclose(min_remainder, 0.0, atol=1e-5, err_msg="Sampled K is not aligned with K_step grid")