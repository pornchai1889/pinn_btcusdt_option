...
def get_timeframe_params(filename, days=1):
    ...
    try:
        ...
        tf_map = {"15m": 0.25, "30m": 0.5, "1h": 1.0, "2h": 2.0, "4h": 4.0, "1d": 24.0}
...
def process_file(
    ...
    sigma_data = calculate_dynamic_volatility(
        S_data, lookback_prices, window_size, ann_factor
    )
    ...
    # --- Graph 1: Prices ---
    ax1.plot(
        t_data, V_market, label="Market Price", color="purple", alpha=0.5, linewidth=1.5
    )
    ax1.plot(
        t_data,
        V_analytical,
    ...
    ax1.set_title(
        f"PINN Model Evaluation: {file_basename}\n"
        f"Vs Market: RMSE={rmse_mkt:.2f}, R={corr_mkt:.4f}  |  "
        f"Vs Theory: RMSE={rmse_anal:.2f}, R={corr_anal:.4f}",
        fontsize=14,
    )
    ...
    ax2 = ax1.twinx()
    ax2.plot(
        t_data, S_data, label="BTC Price", color="green", alpha=0.15, linestyle=":"
    )
    ...
    vol_label = (
        f"Historical Volatility: {LOOKBACK_DAYS}-Day "
    ...
    if append_time_to_filename:
        ...
        save_filename = (
            f"result_{file_basename}_sigma{LOOKBACK_DAYS}day_{timestamp_suffix}.png"
        )
    else:
        # กรณี Batch ชื่อโฟลเดอร์ระบุเวลาไว้แล้ว
        save_filename = (
            f"result_{file_basename}_sigma{LOOKBACK_DAYS}day_r{RISK_FREE_RATE}.png"
        )
...
def main():
    ...
    if config_path is None:
        print(
            f"Error: 'config.json' not found in hierarchy starting from: {RUN_FOLDER}"
        )
        return
    ...
    model = UniversalPINN(
        TRAIN_CONFIG["model"]["n_input"],
        TRAIN_CONFIG["model"]["n_output"],
        TRAIN_CONFIG["model"]["n_hidden"],
        TRAIN_CONFIG["model"]["n_layers"],
    ).to(device)
