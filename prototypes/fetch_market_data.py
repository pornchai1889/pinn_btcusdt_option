...
def get_expiration_type(symbol: str) -> str:
    try:
        ...
        if is_last_friday_of_month and is_quarterly_month:
            return "Quarterly"
        elif is_last_friday_of_month:
            return "Monthly"
        elif is_friday:
            return "Weekly"
        else:
...
def calculate_contract_period(symbol: str, exp_type: str) -> tuple[int, int]:
    ...
    if start_date:
        ...
        return int(aware_start.astimezone(utc_tz).timestamp() * 1000), int(
            aware_end_dt_ict.astimezone(utc_tz).timestamp() * 1000
        )
    return None, None
...
def fetch_klines_forward_robust(url, symbol, interval, start_ms, end_ms):
    ...
    print(
        f"   Target: {datetime.fromtimestamp(start_ms/1000)} -> {datetime.fromtimestamp(end_ms/1000)}"
    )
    ...
    while True:
        ...
        try:
            ...
            print(
                f"   ...fetched batch ending {datetime.fromtimestamp(last_close/1000)}. New items: {new_count} (Total: {len(unique_data_map)})"
            )
            ...
            if next_start <= current_start:
                ...
                if "m" in interval:
                    delta = int(interval.replace("m", "")) * 60 * 1000
                elif "h" in interval:
                    delta = int(interval.replace("h", "")) * 60 * 60 * 1000
                else:
                    delta = 3600000
                current_start += delta
            else:
    ...
    sorted_data = sorted(
        unique_data_map.values(),
        key=lambda x: int(x["openTime"]) if isinstance(x, dict) else int(x[0]),
    )
    return sorted_data
...
def main():
    try:
        strike_price_K = TARGET_OPTION_SYMBOL.split("-")[2]
    except:
        return
    ...
    options_data = fetch_klines_forward_robust(
        option_url, TARGET_OPTION_SYMBOL, TIME_INTERVAL, start_ms, fetch_end_ms
    )
    ...
    print(
        f"   Range: {datetime.fromtimestamp(real_start/1000)} -> {datetime.fromtimestamp(real_end/1000)}"
    )

    # 2. Fetch BTC Data
    print("\n2. Downloading BTC Spot Klines...")
    btc_url = "https://api.binance.com/api/v3/klines"
    btc_data = fetch_klines_forward_robust(
        btc_url, "BTCUSDT", TIME_INTERVAL, real_start, real_end
    )
    ...
    with open(file_path, "w", newline="", encoding="utf-8") as f:
        ...
        for k in options_data:
            ...
            writer.writerow(
                [
                    datetime.fromtimestamp(open_ms / 1000).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    o,
                    h,
                    ...
                    c,
                    v,
                    datetime.fromtimestamp(close_ms / 1000).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    amt,
                    trd,
