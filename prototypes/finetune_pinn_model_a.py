...
def main():
    ...
    if config_path is None:
        print(
            f"Error: 'config.json' not found in hierarchy starting from: {BASE_RUN_DIR}"
        )
        return
    ...
    if "time_sampling_power" in FT_CONFIG:
        MOTHER_CONFIG["sampling"]["time_sampling_power"] = FT_CONFIG[
            "time_sampling_power"
        ]
    ...
    model = UniversalPINN(
        N_INPUT,
        1,
        MOTHER_CONFIG["model"]["n_hidden"],
        MOTHER_CONFIG["model"]["n_layers"],
    ).to(DEVICE)
    ...
    try:
        ...
        # Save Model
        final_model_path = os.path.join(ft_result_dir, "model.pth")
