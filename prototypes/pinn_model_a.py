...
def plot_pre_training(result_dir, config):
    ...
    plt.plot(
        ...
        color="#333333",
        linewidth=2.5,
        label=rf"Base Gaussian ($\sigma$={std_val:.4f})",
        zorder=10,
    )
    ...
    for start_sd, end_sd, color, h_ratio in zones:
        ...
        if end_sd > 3:
            label_txt = rf">{start_sd}$\sigma$ (Tails)"

        # Add to legend list
        from matplotlib.patches import Patch

        zone_legend_handles.append(
            Patch(facecolor=color, edgecolor="none", label=label_txt)
        )
    ...
    # Combine handles
    final_handles = zone_legend_handles[:3] + [
        separator,
        gaussian_summary,
        water_summary,
    ]
    ...
    fix_K = (
        np.round(fix_K_mid / config["market"]["K_step"]) * config["market"]["K_step"]
    )
    ...
    plt.scatter(
        t_pde, S_pde, c="#cccccc", s=10, alpha=0.4, label="PDE Collocation Points"
    )
    ...
    plt.scatter(
        ...
        s=25,
        alpha=0.6,
        label=f"BVP Upper (S={fix_K * m_max:.0f})",
    )
    ...
    plt.scatter(
        ...
        s=25,
        alpha=0.6,
        label=f"BVP Lower (S={fix_K * m_min:.0f})",
    )
    ...
    plt.axhline(
        ...
        linewidth=1.5,
        alpha=0.5,
        label=f"Strike K={fix_K:,.0f}",
    )

    plt.title(
        rf"Data Sampling Distribution (Mixed Moneyness + Power Law Time)", fontsize=14
    )
    plt.xlabel("Time to Maturity (t)")
    plt.ylabel("Spot Price (S)")

    info_text = (
        f"Point Counts (Total: {total_points:,}):\n"
        f"PDE: {n_pde:,}\n"
        f"IVP: {n_data:,}\n"
        f"BVP: {n_data*2:,}\n"
...
def plot_checkpoint_performance(model, config, save_dir, device):
    ...
    # Normalization
    c_m = config["market"]
    t_norm = (X_flat[:, 0] - c_m["t_range"][0]) / (
        c_m["t_range"][1] - c_m["t_range"][0]
    )
    S_norm = (X_flat[:, 1] - c_m["S_range"][0]) / (
        c_m["S_range"][1] - c_m["S_range"][0]
    )
    sig_norm = (X_flat[:, 2] - c_m["sigma_range"][0]) / (
        c_m["sigma_range"][1] - c_m["sigma_range"][0]
    )
    r_norm = (X_flat[:, 3] - c_m["r_range"][0]) / (
        c_m["r_range"][1] - c_m["r_range"][0]
    )
    K_norm = (X_flat[:, 4] - c_m["K_range"][0]) / (
        c_m["K_range"][1] - c_m["K_range"][0]
    )
    ...
    surf1 = ax1.plot_surface(
        t_grid, S_grid, V_true, cmap="viridis", edgecolor="none", alpha=0.9
    )
    ...
    surf2 = ax2.plot_surface(
        t_grid, S_grid, V_pred_grid, cmap="viridis", edgecolor="none", alpha=0.9
    )
    ...
    plt.title(
        f"PINN vs. Analytical Predictions\n(RMSE: {rmse:.4f}, R: {corr:.4f})\n{param_text}",
        fontsize=12,
    )
...
def plot_post_training(result_dir, history):
    ...
    if len(history["total"]) > 0:
        ...
        plot_metric(
            axes[0], history["total"], "#1f77b4", "Total Loss", "Total Loss", "Loss"
        )
        plot_metric(
            axes[1],
            history["pde"],
        ...
        plot_metric(
            ...
            "#d62728",
            "IVP Loss",
            "Initial Value Problem (t=0) Loss",
            "IVP Loss",
        )
        plot_metric(
            axes[4],
        ...
        plt.plot(
            epochs, history["total"], color="#d62728", linewidth=1.5, label="Total Loss"
        )
...
def main():
    ...
    CONFIG = {
        ...
        "validation_params": {"fix_sigma": 0.5, "fix_r": 0.05},
        "model": {"n_input": 5, "n_output": 1, "n_hidden": 256, "n_layers": 4},
        "training": {
            "epochs": 600000,
            "lr": 1e-4,
            "n_sample_data": 10000,
    ...
    logging.info(
        f"Adaptive Factor: {ADAPTIVE_STD_FACTOR} => Calculated Real Sigma: {REAL_STD:.6f}"
    )
    logging.info(
        "Sampling Strategy: Gaussian + Uniform Resampling for Outliers (Mixed Distribution)"
    )
    ...
    model = UniversalPINN(
        N_INPUT, 1, CONFIG["model"]["n_hidden"], CONFIG["model"]["n_layers"]
    ).to(DEVICE)
    ...
    try:
        for i in tqdm(range(EPOCHS), desc="Training PINN", unit="epoch"):
            ...
            X_pde_tensor = (
                torch.from_numpy(X_pde_norm).float().to(DEVICE).requires_grad_()
            )
            ...
            pde_res = dV_dt - (
                0.5 * sigma_pde**2 * S_pde**2 * d2V_dS2
                + r_pde * S_pde * dV_dS
                - r_pde * V_real
            )
            pde_loss = PHYSICS_WEIGHT * loss_fn(
                pde_res / K_pde, torch.zeros_like(pde_res)
            )
