...
def locate_mother_config_and_root(base_search_dir: str) -> Tuple[str, str]:
    ...
    if config_path is None:
        raise FileNotFoundError(
            f"Error: 'config.yaml' not found in hierarchy starting from: {base_search_dir}"
        )
    ...
    normalized_anchor = anchor_dir.replace(
        "\\", "/"
    )  # Normalize for consistent splitting
...
def main():
    ...
    try:
        ...
        root_run_dir, mother_config_path = locate_mother_config_and_root(
            target_base_dir
        )
        ...
        for k, v in ft_params.items():
            if k in config["training"]:
                ...
                logging.warning(
                    f"Key '{k}' not found in training/sampling config. Adding to 'training'."
                )
                config["training"][k] = v
        ...
        if not os.path.exists(checkpoint_path):
            ...
            fallback_path = os.path.join(
                target_base_dir, "checkpoints", ft_config["model_checkpoint"]
            )
