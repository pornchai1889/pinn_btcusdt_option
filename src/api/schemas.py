...
class GreeksResponse(BaseModel):
    ...
    """
    delta: float = Field(..., description="Delta: First-order sensitivity to Spot Price (dV/dS).")
    gamma: float = Field(..., description="Gamma: Second-order sensitivity to Spot Price (d2V/dS2). Convexity.")
...
class PricingResponse(BaseModel):
    ...
    """
    price: float = Field(..., description="Predicted Option Price (V) from the PINN model.")
    greeks: GreeksResponse = Field(..., description="Associated Greek risk metrics derived via Autograd.")
    
    # Metadata for System Monitoring & Reproducibility
    model_version: str = Field(..., description="Checkpoint identifier of the loaded model.")
    inference_time_ms: float = Field(..., description="Total inference latency in milliseconds (ms).")
    device: str = Field(..., description="Compute unit used for inference (e.g., 'cpu', 'cuda:0').")
