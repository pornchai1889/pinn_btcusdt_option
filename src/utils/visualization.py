...
class Visualizer:
    ...
    def __init__(
        self, config: Dict[str, Any], physics_engine: Any, run_dir: Optional[str] = None
    ):
        """
        Args:
    ...
    def plot_pre_training(self, save_dir: str) -> None:
        ...
        for i in range(1, 4):
            for sign in [1, -1]:
                ...
                if m_min < sd_pos < m_max:
                    plt.axvline(
                        sd_pos, color="grey", linestyle=":", alpha=0.5, linewidth=1
                    )
        ...
        plt.legend(
            ...
            loc="upper right",
            framealpha=0.95,
            title="Data Distribution Statistics",
        )
        ...
        plt.scatter(
            t_pde, S_pde, c="#858484", s=10, alpha=0.4, label="PDE Collocation Points"
        )
    ...
    def plot_checkpoint_performance(
        ...
        fig.suptitle(
            f"Option Price Surface Comparison (Epoch {epoch})\n{param_text}",
            fontsize=14,
            y=0.98,
        )
        ...
        plt.scatter(
            v_pred_flat, v_true_flat, alpha=0.5, s=10, label="Prediction Points"
        )
        ...
        corr = (
            np.corrcoef(v_true_flat, v_pred_flat)[0, 1]
            if np.std(v_true_flat) > 0 and np.std(v_pred_flat) > 0
            else 0
        )

        plt.title(
            f"Parity Plot (Epoch {epoch})\nRMSE: {rmse:.4f}, Corr: {corr:.4f}\n({param_text})",
            fontsize=12,
        )
        ...
        # Normalize Slice
        t_norm_s = (X_slice[:, 0] - c_m["t_range"][0]) / (
            c_m["t_range"][1] - c_m["t_range"][0]
        )
        S_norm_s = (X_slice[:, 1] - c_m["S_range"][0]) / (
            c_m["S_range"][1] - c_m["S_range"][0]
        )
        ...
        V_slice_true = self.physics.analytical_solution(
            t_slice_raw, S_slice_raw, fix_K, fix_r, fix_sig
        )
        ...
        metric_text = (
            f"Pointwise Abs. Error at Strike ($S=K$): {kink_error:.5f}\n({param_text})"
        )
    ...
    def plot_loss_history(
        self, history: Dict[str, List[float]], save_dir: Optional[str] = None
    ) -> None:
        """
        ...
        if len(history["total"]) > 0:
            ...
            plot_metric(
                ...
                "#d62728",
                "IVP Loss",
                r"Initial Value Points ($\tau=0$) Loss",
                "IVP Loss",
            )
    ...
    def _sample_moneyness_mixed(
        self, n: int, m_min: float, m_max: float, std_val: float
    ) -> np.ndarray:
        """
