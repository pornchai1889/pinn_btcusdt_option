...
class BinanceDataFetcher:
    ...
    def fetch_klines(
        self, url: str, symbol: str, interval: str, start_ms: int, end_ms: int
    ) -> List[Any]:
        """
        ...
        self.logger.info(
            f"Fetching {symbol} ({interval}): {datetime.fromtimestamp(start_ms/1000)} -> {datetime.fromtimestamp(end_ms/1000)}"
        )
        ...
        while True:
            ...
            try:
                ...
                self.logger.debug(
                    f"Fetched batch ending {datetime.fromtimestamp(last_close_time/1000)}. New: {new_count}"
                )
    ...
    def _parse_interval(self, interval: str) -> int:
        ...
        if "m" in interval:
            return int(interval.replace("m", "")) * 60 * 1000
        elif "h" in interval:
            return int(interval.replace("h", "")) * 60 * 60 * 1000
        elif "d" in interval:
            return int(interval.replace("d", "")) * 24 * 60 * 60 * 1000
