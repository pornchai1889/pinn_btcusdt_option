...
class MarketDataHandler:
    ...
    def get_timeframe_params(self, filename: str) -> Tuple[str, int, float]:
        ...
        try:
            ...
            if timeframe_str not in tf_map:
                logger.warning(f"Unknown timeframe '{timeframe_str}'. Defaulting to 1h.")
    ...
    def fetch_btc_lookback(self, start_time_ms: int, timeframe_str: str, limit: int) -> List[float]:
        """
        ...
        while remaining > 0:
            ...
            try:
                ...
                if len(data) < batch_size: break # End of data
...
class RealMarketEvaluator:
    ...
    def __init__(self, config_path: str):
        ...
        # Setup paths
        self.run_folder = os.path.join(project_root, self.eval_config['paths']['run_folder'])
    ...
    def _load_train_config(self) -> dict:
        ...
        for _ in range(5):
            ...
            if os.path.exists(p_json):
                with open(p_json, 'r') as f: return json.load(f)
            if os.path.exists(p_yaml):
                with open(p_yaml, 'r') as f: return yaml.safe_load(f)
    ...
    def run(self):
        ...
        logger.info(f"Found {len(files)} CSV files in {data_dir}. Starting processing...")
    ...
    def process_file(self, csv_path: str):
        ...
        required = ['btc_close_price', 'time_to_maturity_t2m', 'strike_price_K', 'close', 'open_time_unix_ms']
        ...
        sigma_data = self.data_helper.calculate_dynamic_volatility(S_data, lookback, window, ann_factor)
        r_data = np.full_like(S_data, self.data_helper.r)

        # 5. Normalize & Predict
        X_input = self.normalizer.normalize_batch(
            t_data.reshape(-1,1), S_data.reshape(-1,1), 
            sigma_data.reshape(-1,1), r_data.reshape(-1,1), K_data.reshape(-1,1)
        )
        ...
        V_analytical = physics.analytical_solution(t_data, S_data, K_data, r_data, sigma_data)

        # 7. Metrics
        rmse_anal = np.sqrt(np.mean((V_analytical - V_pred_pinn)**2))
        corr_anal = np.corrcoef(V_analytical, V_pred_pinn)[0, 1] if np.std(V_pred_pinn) > 0 else 0
        ...
        if can_compare_market:
            ...
            corr_mkt = np.corrcoef(V_market, V_pred_pinn)[0, 1] if np.std(V_pred_pinn) > 0 else 0
        else:
            rmse_mkt, corr_mkt = 0.0, 0.0

        # 8. Visualization
        self._generate_plot(
            t_data, S_data, V_market, V_analytical, V_pred_pinn, sigma_data,
    ...
    def _generate_plot(self, t_data, S_data, V_market, V_analytical, V_pred, sigma_data,
        ...
        ax1.plot(t_data, V_pred, label='PINN Prediction', color='darkorange', linestyle='--', linewidth=2)
        ...
        title_str = (f'Evaluation: {file_basename}\n'
        ...
        ax2 = ax1.twinx()
        ax2.plot(t_data, S_data, label='BTC Price', color='green', alpha=0.3, linestyle=':')
        ...
        vol_label = (rf"Historical $\sigma$: {self.data_helper.lookback_days}-Day "
